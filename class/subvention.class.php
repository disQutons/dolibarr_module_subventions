...
/**
 * \file        class/subvention.class.php
 * \ingroup     subventions
...
/**
 * Class for Subvention
 */
class Subvention extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'subvention';

	/**
	 * @var string 	Name of table without prefix where object is stored. This is also the key used for extrafields management (so extrafields know the link to the parent table).
	 */
	public $table_element = 'subventions_subvention';

	/**
	 * @var string 	If permission must be checkec with hasRight('subventions', 'read') and not hasright('mymodyle', 'subvention', 'read'), you can uncomment this line
	 */
	//public $element_for_permission = 'subventions';

	/**
	 * @var string 	String with name of icon for subvention. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'subvention@subventions' if picto is file 'img/object_subvention.png'.
	 */
	public $picto = 'fa-hand-holding-heart';


	const STATUS_DRAFT = 0; //Non déposé
	const STATUS_VALIDATED = 1; //Déposé
	const STATUS_ACCEPTED = 2; //Accepté
	const STATUS_FINANCED = 3; //Financé
	...
	public $description;
	public $evaluation;
	public $date_d_projet;
	public $date_f_projet;
	public $date_attendue;
	public $date_bilan;
	public $note_public;
	public $note_private;
	...
	//  */
	// public $table_element_line = 'subventions_subventionline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_subvention';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Subventionline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Subvention', 'fk_element'=>'fk_subvention'));
	...
	//  */
	// protected $childtablesoncascade = array('subventions_subventiondet');

	// /**
	//  * @var SubventionLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('subventions', 'subvention', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('subventions', 'subvention_advance', 'validate')))
		 {
		...
		if (!empty($num)) {
			...
			if (!$error && !$notrigger) {
				// Call trigger
				$result = $this->call_trigger('SUBVENTION_VALIDATE', $user);
				if ($result < 0) {
					$error++;
				}
				// End call triggers
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'subvention/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'subvention/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->subventions->dir_output.'/subvention/'.$oldref;
				$dirdest = $conf->subventions->dir_output.'/subvention/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->subventions->dir_output.'/subvention/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'SUBVENTIONS_SUBVENTION_UNVALIDATE');
	}
    ...
    public function accepted($user, $notrigger = 0)
	{
		...
		return majstatut($this);
	}
    public function refuse($user, $notrigger = 0)
	{
		refuseSub($this);
		$this->setStatusCommon($user, self::STATUS_REFUSED, $notrigger, '');
		return majstatut($this);
	}
    public function evaluate($user, $notrigger = 0)
	{
		$this->setStatusCommon($user, self::STATUS_EVALUATED, $notrigger, '');
		return majstatut($this);
	}
    public function setfinanced($user, $notrigger = 0)
	{
		$this->setStatusCommon($user, self::STATUS_FINANCED, $notrigger, '');
		return majstatut($this);
	}
    public function cloture($user, $notrigger = 0)
	{
		$this->setStatusCommon($user, self::STATUS_CLOTURED, $notrigger, '');
		return majstatut($this);
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		//return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'SUBVENTIONS_SUBVENTION_REOPEN');
		return majstatut($this);
	}
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowSubsidy")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Subsidy").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/subventions/subvention_card.php', 1).'?id='.$this->id;
	...
	public function getLinesArray()
	{
		...
		$objectline = new SubventionLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_subvention:=:'.((int) $this->id).')');
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('SUBVENTIONS_SUBVENTION_ADDON')) {
			$conf->global->SUBVENTIONS_SUBVENTION_ADDON = 'mod_subvention_standard';
		}

		if (getDolGlobalString('SUBVENTIONS_SUBVENTION_ADDON')) {
			$mybool = false;

			$file = getDolGlobalString('SUBVENTIONS_SUBVENTION_ADDON').".php";
			$classname = getDolGlobalString('SUBVENTIONS_SUBVENTION_ADDON');
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			...
			} elseif (getDolGlobalString('SUBVENTION_ADDON_PDF')) {
...
/**
 * Class SubventionLine. You can also remove this and generate a CRUD class for lines objects.
 */
class SubventionLine extends CommonObjectLine
{
	// To complete with content of an object SubventionLine
	// We should have a field rowid, fk_subvention and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $parent_element = '';		// Example: '' or 'subvention'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_subvention'
