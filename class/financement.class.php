...
/**
 * \file        class/financement.class.php
 * \ingroup     subventions
 * \brief       This file is a CRUD class file for Financement (Create/Read/Update/Delete)
 */
 ...
 */
class Financement extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'financement';

	/**
	 * @var string 	Name of table without prefix where object is stored. This is also the key used for extrafields management (so extrafields know the link to the parent table).
	 */
	public $table_element = 'subventions_financement';

	/**
	 * @var string 	If permission must be checkec with hasRight('subventions', 'read') and not hasright('mymodyle', 'financement', 'read'), you can uncomment this line
	 */
	//public $element_for_permission = 'subventions';

	/**
	 * @var string 	String with name of icon for financement. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'financement@subventions' if picto is file 'img/object_financement.png'.
	 */
	...
	public $fields = array(
		...
		"ref" => array("type" => "varchar(128)", "label" => "Ref", "enabled" => "1", 'position' => 20, 'notnull' => 1, "visible" => "4", "default" => "(PROV)", "index" => "1", "searchall" => "1", "validate" => "1", "comment" => "Reference of object"),
	...
	//  */
	// public $table_element_line = 'subventions_financementline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_financement';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Financementline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Financement', 'fk_element'=>'fk_financement'));
	...
	//  */
	// protected $childtablesoncascade = array('subventions_financementdet');

	// /**
	//  * @var FinancementLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		// Récupération des données de ma liste de financeurs
		$sql = 'SELECT c.rowid, c.ref, c.label, c.active, c.position';
        ...
        if ($resql)
        {
            ...
            if ($num > 0)
            {
                ...
                $this->fields['fk_financeur']['arrayofkeyval'] = $arrayofkeyval;
            }
            $db->free($resql);
        }
        else
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('subventions', 'financement', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('subventions', 'financement_advance', 'validate')))
		 {
		...
		if (!empty($num)) {
			...
			if (!$error && !$notrigger) {
				// Call trigger
				$result = $this->call_trigger('FINANCEMENT_VALIDATE', $user);
				if ($result < 0) {
					$error++;
				}
				// End call triggers
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'financement/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'financement/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->subventions->dir_output.'/financement/'.$oldref;
				$dirdest = $conf->subventions->dir_output.'/financement/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->subventions->dir_output.'/financement/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'SUBVENTIONS_FINANCEMENT_CANCEL');
	}
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowFunding")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Funding").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/subventions/financement_card.php', 1).'?id='.$this->id;
	...
	public function getLinesArray()
	{
		...
		$objectline = new FinancementLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_financement:=:'.((int) $this->id).')');
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('SUBVENTIONS_FINANCEMENT_ADDON')) {
			$conf->global->SUBVENTIONS_FINANCEMENT_ADDON = 'mod_financement_standard';
		}

		if (getDolGlobalString('SUBVENTIONS_FINANCEMENT_ADDON')) {
			$mybool = false;

			$file = getDolGlobalString('SUBVENTIONS_FINANCEMENT_ADDON').".php";
			$classname = getDolGlobalString('SUBVENTIONS_FINANCEMENT_ADDON');
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			...
			} elseif (getDolGlobalString('FINANCEMENT_ADDON_PDF')) {
				$modele = getDolGlobalString('FINANCEMENT_ADDON_PDF');
			}
		}
...
/**
 * Class FinancementLine. You can also remove this and generate a CRUD class for lines objects.
 */
class FinancementLine extends CommonObjectLine
{
	// To complete with content of an object FinancementLine
	// We should have a field rowid, fk_financement and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $parent_element = '';		// Example: '' or 'financement'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_financement'
