...
/**
 *   	\file       subvention_list.php
 *		\ingroup    subventions
 *		\brief      List page for subvention
 */
...
require_once __DIR__.'/class/subvention.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('subventions', 'subvention', 'write');
	$permissiontodelete = $user->hasRight('subventions', 'subvention', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Subvention';
	$objectlabel = 'Subvention';
	$uploaddir = $conf->subventions->dir_output;
...
$title = $langs->trans("Subsidys");
//$help_url = "EN:Module_Subvention|FR:Module_Subvention_FR|ES:Módulo_Subvention";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/subventions/subvention_card.php', 1).'?id='.((int) $id));
	exit;
}
...
if ($socid && $user->hasRight('societe', 'lire')) {
	...
	if ($res > 0) {
		...
		print dol_get_fiche_head($head, 'subventions', $langs->trans("ThirdParty"), -1, 'company');
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/subventions/subvention_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, $object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendSubventionRef";
$modelmail = "subvention";
$objecttmp = new Subvention($db);
...
if ($search_all) {
	...
	print '<!-- Search done like if SUBVENTION_QUICKSEARCH_ON_FIELDS = '.$setupstring.' -->'."\n";
