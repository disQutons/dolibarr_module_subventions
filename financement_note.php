...
/**
 *  \file       financement_note.php
 *  \ingroup    subventions
 *  \brief      Tab for notes on Financement
 */
...
$langs->loadLangs(array("subventions@subventions", "companies"));
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->subventions->multidir_output[empty($object->entity) ? $conf->entity : $object->entity]."/".$object->id;
}
...
$title = $langs->trans('Funding').' - '.$langs->trans("Notes");
...
if ($id > 0 || !empty($ref)) {
	...
	print dol_get_fiche_head($head, 'note', $langs->trans("Funding"), -1, $object->picto);
