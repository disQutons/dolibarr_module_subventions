...
/**
 *  \file       paiement_document.php
 *  \ingroup    subventions
 *  \brief      Tab for documents linked to Paiement
 */
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->subventions->multidir_output[$object->entity ? $object->entity : $conf->entity]."/paiement/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd  = $user->hasRight('subventions', 'paiement', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd  = 1;
}
...
// Header
// ------
$title = $langs->trans("Payment")." - ".$langs->trans("Files");
...
print dol_get_fiche_head($head, 'document', $langs->trans("Payment"), -1, $object->picto);
...
$param = '&id='.$object->id;
//$relativepathwithnofile='paiement/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'paiement/'.dol_sanitizeFileName($object->ref).'/';
