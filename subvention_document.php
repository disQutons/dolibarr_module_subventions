...
/**
 *  \file       subvention_document.php
 *  \ingroup    subventions
 *  \brief      Tab for documents linked to Subvention
 */
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->subventions->multidir_output[$object->entity ? $object->entity : $conf->entity]."/subvention/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd  = $user->hasRight('subventions', 'subvention', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd  = 1;
}
...
// Header
// ------
$title = $langs->trans("Subsidy")." - ".$langs->trans("Files");
...
print dol_get_fiche_head($head, 'document', $langs->trans("Subsidy"), -1, $object->picto);
...
$param = '&id='.$object->id;
//$relativepathwithnofile='subvention/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'subvention/'.dol_sanitizeFileName($object->ref).'/';
