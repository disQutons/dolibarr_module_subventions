...
/**
 *  \file       financement_document.php
 *  \ingroup    subventions
 *  \brief      Tab for documents linked to Financement
 */
...
$langs->loadLangs(array("subventions@subventions", "companies", "other", "mails"));
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->subventions->multidir_output[$object->entity ? $object->entity : $conf->entity]."/financement/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd  = $user->hasRight('subventions', 'financement', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd  = 1;
}
...
// Header
// ------
$title = $langs->trans("Funding")." - ".$langs->trans("Files");
//$title = $object->ref." - ".$langs->trans("Files");
$help_url = '';
//Example $help_url='EN:Module_Third_Parties|FR:Module_Tiers|ES:Empresas';
llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'mod-subventions page-card_document');

// Show tabs
$head = financementPrepareHead($object);

print dol_get_fiche_head($head, 'document', $langs->trans("Funding"), -1, $object->picto);
...
$modulepart = 'subventions';
$param = '&id='.$object->id;
//$relativepathwithnofile='financement/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'financement/'.dol_sanitizeFileName($object->ref).'/';
